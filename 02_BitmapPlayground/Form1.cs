...
namespace _02_BitmapPlayground
{
    public partial class Form1 : Form
    {
        ...
        {
           var filters = GetInstances<IFilter>();
        ...
        {
            if (FilterPickerBox.SelectedItem is IFilter)
            {
                // using thread pooling
                ThreadPool.QueueUserWorkItem(Process, new object[] { FilterPickerBox.SelectedItem, PictureBoxOriginal.Image });               

            }            
        }

        void Process(object parameter)
        {
            object[] objArray = parameter as object[];
            Image image = ApplyFilter(objArray[0] as IFilter, objArray[1] as Image);
            PictureBoxFiltered.Image = image;
        }
